...
public class HtmlForMarkdown {
     ...
     */
    public HtmlForMarkdown(final AbstractDocbkxMojo mojo) {
        m = mojo;
    ...
    final String getTargetDB() throws MojoExecutionException {
        File targetDB = new File(m.getBuildDirectory(), "olinkdb-markdown.xml");

        try {
            OLinkUtils.createTargetDatabase(targetDB, "markdown", m, true);
        } catch (Exception e) {
    ...
    class Executor extends MojoExecutor {
        ...
        void prepareOlinkDB() throws MojoExecutionException {
            ...
            for (String docName : m.getDocNames()) {
                ArrayList<Element> cfg = new ArrayList<>();
                ...
                cfg.add(element(name("htmlCustomization"), m.path(m.getMarkdownCustomization())));
                ...
                cfg.add(element(name("targetDirectory"), m.path(m.getDocbkxOutputDirectory()) + "/markdown"));
                cfg.add(element(name("targetsFilename"), m.getDocumentSrcName() + ".markdown.target.db"));
        ...
        void build() throws MojoExecutionException {
            ...
            try {
                ImageCopier.copyImages("markdown", "", m);
            } catch (IOException e) {
